...
public class GlobalExceptionHandler {
    ...
    private ResponseEntity<ApiError> build(HttpStatus status, String error, String message,
            HttpServletRequest request, String errorCode, List<ApiError.FieldError> fieldErrors) {
        String reqId = errorCode != null ? errorCode : request.getHeader("X-Request-ID");
        ApiError body = ApiError.builder()
            .status(status.value()).error(error).message(message)
            .path(request.getRequestURI()).requestId(reqId)
