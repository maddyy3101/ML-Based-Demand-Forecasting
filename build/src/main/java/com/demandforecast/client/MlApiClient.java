...
public class MlApiClient {
    ...
    public Mono<Double> predict(MlPredictPayload payload, String requestId) {
        ObjectNode body = buildBody(payload);
        ...
        return webClient.post()
            ...
            .onStatus(HttpStatusCode::is4xxClientError, resp ->
                resp.bodyToMono(String.class)
                    .map(b -> new MlApiException("ML API rejected request (4xx): " + b)))
            .onStatus(HttpStatusCode::is5xxServerError, resp ->
                resp.bodyToMono(String.class)
                    .map(b -> new MlApiUnavailableException(new RuntimeException(b))))
            .bodyToMono(JsonNode.class)
            .map(json -> json.get("demand").asDouble())
            .retryWhen(Retry.backoff(2, Duration.ofMillis(300))
                .filter(ex -> ex instanceof WebClientRequestException)
                .onRetryExhaustedThrow((spec, sig) ->
                    new MlApiUnavailableException(sig.failure())))
    ...
    public Mono<List<Double>> predictBatch(List<MlPredictPayload> payloads, String requestId) {
        List<ObjectNode> bodies = payloads.stream().map(this::buildBody).toList();
    ...
    public Mono<Map<String, Double>> getFeatureImportance() {
        return webClient.get()
            ...
            .bodyToFlux(JsonNode.class)
            .collectMap(
                node -> node.get("feature").asText(),
                node -> node.get("importance").asDouble()
            );
    }
